...
def transform_quantiles_to_dataframe(
    titles_quantiles_dict: dict[str, dict[int, float]],
) -> pd.DataFrame:
    """
...
def analyze_breakpoints(df_quantiles: pd.DataFrame) -> pd.DataFrame:
    ...
    df_thresholds["First Derivative Threshold"] = max_first_diff_idx.apply(
        lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
        if df_quantiles.columns.get_loc(x) > 0
        else df_quantiles.columns[0]
    )

    # Calculate the second-highest point for the first derivative
    second_max_first_diff_idx = first_diff.apply(
        lambda row: row.nlargest(2).idxmin(), axis=1
    )
    df_thresholds["Second Threshold First Derivative"] = (
        second_max_first_diff_idx.apply(
            lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
            if df_quantiles.columns.get_loc(x) > 0
            else df_quantiles.columns[0]
        )
    ...
    third_max_first_diff_idx = first_diff.apply(
        lambda row: row.nlargest(3).idxmin(), axis=1
    )
    df_thresholds["Third Threshold First Derivative"] = third_max_first_diff_idx.apply(
        lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
        if df_quantiles.columns.get_loc(x) > 0
        else df_quantiles.columns[0]
    )
    ...
    df_thresholds["Second Derivative Threshold"] = max_second_diff_idx.apply(
        lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
        if df_quantiles.columns.get_loc(x) > 0
        else df_quantiles.columns[0]
    )
    ...
    second_max_second_diff_idx = second_diff.apply(
        lambda row: row.nlargest(2).idxmin(), axis=1
    )
    df_thresholds["Second Threshold Second Derivative"] = (
        second_max_second_diff_idx.apply(
            lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
            if df_quantiles.columns.get_loc(x) > 0
            else df_quantiles.columns[0]
        )
    ...
    third_max_second_diff_idx = second_diff.apply(
        lambda row: row.nlargest(3).idxmin(), axis=1
    )
    df_thresholds["Third Threshold Second Derivative"] = (
        third_max_second_diff_idx.apply(
            lambda x: df_quantiles.columns[df_quantiles.columns.get_loc(x) - 1]
            if df_quantiles.columns.get_loc(x) > 0
            else df_quantiles.columns[0]
        )
...
def extract_average_significant_quantiles(
    df_quantiles: pd.DataFrame, df_rupture_threshold: pd.DataFrame
) -> pd.Series:
    """
    ...
    average_quantiles_series = pd.Series(
        average_quantiles_dict, name="Average Significant Quantiles"
    )
...
def calculate_degradation_score(
    average_quantiles_series: pd.Series,
    reference_value: float = 22000,
    title_order: bool = False,
) -> pd.Series:
    """
    ...
    percentage_diff = (
        (average_quantiles_series - reference_value) / reference_value
    ) * 100
    ...
    if title_order:
        ...
        degradation_score_sorted = degradation_score_rounded.sort_values(
            ascending=False
        )
...
def analyze_audio_files(
    ...
    for file_path in audio_file_paths:
        ...
        frequency_percentiles = quantify_distribution_points_by_frequency(
            spectrogram_db,
            sample_rate,
            threshold_db=threshold_db_value,
    ...
    average_quantiles_per_title = extract_average_significant_quantiles(
...
def plot_spectrogram(
    ...
    )  # Change the color of the colorbar labels
...
def analyse_and_plot(
    ...
    plot_spectrogram_1: bool = False,
    plot_spectrogram_2: bool = False,
) -> None:
    """
